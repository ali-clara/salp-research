...
def load_and_compile(path, num_trials):
    ...
    return np.array(big_area_list), np.array(big_time_list)
...
def graph_values(values, times, avg, avg_time, stdv):
    ...
    my_plot.set_axis_labels("Time (sec)", "Contour Circumference(mm)")
    my_plot.plot_xy()
    my_plot.set_savefig("avg_donut_circumference.png")
    my_plot.label_and_save()

areas, times = load_and_compile("data/8-31-23", num_trials=3)
circs = np.sqrt(4*np.pi*areas)
average = np.mean(circs, axis=0)
stdv = np.std(circs, axis=0)
...
np.save("average-donut-circumference.npy", average_interp)
